...
| `English`              | Fuerza la interfaz en inglés.                 |
| `Español`              | Fuerza la interfaz en español.                |
| `中文`                 | Fuerza la interfaz en chino (simplificado).   |
| `日本語`               | Fuerza la interfaz en japonés.                |
| `Português (Brasil)`   | Fuerza la interfaz en portugués brasileño.    |
| `Русский`              | Fuerza la interfaz en ruso.                   |
...
:::note
Aunque el valor por defecto en los presets de configuración es `false` (para máxima compatibilidad en dispositivos Android antiguos que ejecutan RPG Maker MV), se recomienda encarecidamente activarlo para un mejor rendimiento si los dispositivos de destino soportan WebGL.
:::
...
Para más detalles sobre cómo funciona el sistema de posicionamiento, consulta la referencia de [Elementos Posicionables](/es/reference/positions/).
...
:::note
Los botones en pantalla no disparan eventos reales de teclado. En su lugar, se envía un pseudo evento básico al WebView que MV/MZ pueden entender para aplicar el comportamiento para dicho evento. Para una lista completa de códigos de tecla soportados, consulta la [Referencia de Teclas de Entrada](/es/reference/input-keys/).
:::
