...
2. **Configure your custom icon**:
   ...
   * Configure the generator properties in `ludens.properties`:
   ...
   If you prefer to manually configure your icons using Android Studio's **Image Asset Studio** (right-click `composeApp/src/androidMain/res` > **New > Image Asset**), you must disable the automatic generator first by setting `ludens.icons.android.enable=false` and `ludens.icons.ios.enable=false` in `ludens.properties`.
   :::
