...
| `Español`              | Forces Spanish UI.                           |
| `中文`                 | Forces Chinese (Simplified) UI.              |
| `日本語`               | Forces Japanese UI.                          |
| `Português (Brasil)`   | Forces Portuguese (Brazil) UI.               |
| `Русский`              | Forces Russian UI.                           |
...
:::note
While the default value in the configuration presets is `false` (for maximum compatibility on older Android devices running RPG Maker MV), it is highly recommended to enable it for better performance if the target devices support WebGL.
:::
...
For more details on how the position system works, see the [Positionable Elements](/0.4.0/reference/positions/) reference.
...
:::note
On-screen buttons do not fire real keyboard events. Instead, they send a basic pseudo-event to the WebView that RPG Maker MV/MZ can understand to apply the behavior for that event. For a complete list of supported key codes, see the [Input Keys Reference](/0.4.0/reference/input-keys/).
:::
