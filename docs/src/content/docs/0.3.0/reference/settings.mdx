...
For more details on how the position system works, see the [Positionable Elements](/0.3.0/reference/positions/) reference.
...
:::note
On-screen buttons do not fire real keyboard events. Instead, they send a basic pseudo-event to the WebView that RPG Maker MV/MZ can understand to apply the behavior for that event. For a complete list of supported key codes, see the [Input Keys Reference](/0.3.0/reference/input-keys/).
:::
