...
By default, the Ludens application is configured to run in Landscape mode (`sensorLandscape`). If your game is designed for Portrait mode, you will need to change this configuration in `ludens.properties`. See the [Android Configuration](/configuration/android/#manifest-configuration) guide for details.
...
<FileTree>
- ludens/
  - shared/ KMP shared module (common code, logic, UI)
    - src/
      - commonMain/
        - composeResources/
          - files/
